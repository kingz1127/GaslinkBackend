...
public class PaymentService {
    ...
    public InitiatePaymentResponse initiatePayment(InitiatePaymentRequest req) {
        ...
        if (order.getPaymentStatus() == PaymentStatus.PAID) throw new BusinessRuleException("Order already paid");
        ...
        Payment p = Payment.builder().orderId(order.getId()).gateway("paystack")
                .gatewayRef(resp.getReference()).amount(order.getTotal()).currency("NGN").build();
    ...
    public void verifyPayment(String reference) {
        ...
        if (ok) {
            ...
            paymentRepository.save(p);
            Order o = orderRepository.findById(p.getOrderId()).orElseThrow();
    ...
    public void confirmCashPayment(UUID orderId) {
        ...
        o.setPaymentStatus(PaymentStatus.PAID);
        orderRepository.save(o);
        paymentRepository.findByOrderId(orderId).ifPresent(p -> { p.setStatus(PaymentStatus.PAID); p.setPaidAt(Instant.now()); paymentRepository.save(p); });
    }
}